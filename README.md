...
- If you want to test it, you can check the Network tab in 'Production' mode(Pre-fetching does not work in 'Development' mode).
- All pages in the app router are divided into the following two categories:
   - Static page: Static pages pre-generated at build time(like SSG). Both the RSC payload and the JS Bundle are delivered.
   - Dynamic page: Pages generated on each browser request(like SSR). Only the RSC payload is delivered.
<br><br>
